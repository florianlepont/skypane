...
## Reconciliation Addendum (2026-08-26, post-execution)

This research was conducted before Waves 3-4 executed. Once Wave 3's illustration work landed and the developer reviewed real rendered previews mid-execution, two of this document's headline recommendations were overturned on the evidence of those previews — decisions D-20 and D-21, both recorded in `03-CONTEXT.md`'s "Post-execution course-correction after reviewing real render previews" section. **This is the research process working as intended, not failing:** a recommendation was made from the best available evidence at planning time, real output was generated, the developer looked at it, and the recommendation was revised in light of what was actually seen. Nothing below deletes or contradicts the original findings above — this section records which recommendations were later overturned, by what evidence, and which findings remain fully load-bearing in the shipped code.

### What was overturned

| Research section | What it recommended | What actually shipped |
|---|---|---|
| "Full Composition Realignment §1" — serif font recommendation | **Zilla Slab**, SemiBold/Bold only, chosen specifically because a slab serif "structurally cannot have thin hairlines" — the entire rationale for the category choice was engineered to avoid the e-ink hairline-legibility risk this same document flags repeatedly | Zilla Slab was rejected on a real preview (D-20); the developer chose **PT Serif** instead — a font this research never evaluated. `PTSerif-Bold` was vendored first; after seeing Bold rendered, the developer asked for a thinner look and confirmed **PT Serif Regular** (D-27), which reintroduces precisely the hairline risk the original Zilla Slab recommendation was engineered to avoid. The developer was shown that exact caveat before confirming Regular and accepted the tradeoff anyway. `server/assets/fonts/VENDOR.md` carries the same flag under PT Serif's "Known risk — Regular weight is active" section. The documented fallback if the panel proves it illegible is the already-vendored `PTSerif-Bold.ttf`, for at least the smallest text roles. |
| "Full Composition Realignment §2" — full-canvas dithered background, and the whole "photographic-toned" direction (Pattern 5, the `build_mood_background()` recipe, `MOOD_RGB`/`LIGHT_TINT_MIX`/`NOISE_AMPLITUDE`) | A vertical-gradient, Floyd-Steinberg-dithered "mood" background per state, texturally similar to a photograph, verified this session to cost only ~47ms of render time regardless of scale | Superseded by **D-21**: after the developer saw the actual dithered/speckled result on a real preview, they did not want the textured look after all — despite D-17 having explicitly anticipated and accepted "photographic-toned, dithered" in the abstract. This was **not a scoping compromise, it was a direct reversal on visual evidence**. `build_mood_background()` and its supporting constants were **removed from `server/plane/dither.py` entirely**, not left dead — confirmed directly in the module's current docstring. The replacement flat background colours — Blue `(110, 180, 225)` and Green `(140, 195, 130)` — were separately previewed and confirmed by the developer as "parfait." |
| "Full Composition Realignment §3" — D-18's state-signal question ("two distinct dithered background moods... as the primary state signal") | Recommended Option (a): two dithered moods, reinforced by state-label text and illustration mirroring as secondary cues | **Moot under a flat background** — there is only one flat colour per state now, not two moods to distinguish. The compounding effect: D-24 (below) also dropped mirroring, removing the *second* non-colour-dependent cue this recommendation leaned on as reinforcement. Background colour plus state-label text are now the *only* departing/arriving signals. This is an accepted tradeoff, not a defect — and it is exactly the question Revision 4's own open item #6 asks Wave 4 to judge on real glass. |
| "Pattern 5" — flat quiet-zone text-compositing rectangles | A `draw.rectangle(fill=bg_idx)` patch behind every caption/label, needed to keep text legible over the (then-anticipated) dithered background | **No quiet zones exist in the shipped renderer.** They existed solely to protect text legibility over a dithered field; with D-21's flat background, there is nothing to protect against. `render.py` has no `QUIET_ZONE_PAD` or `draw_quiet_zone()`. |
| "Real Panel RGB Values" — the panel RGB table, all four inks treated as equally interim/community-sourced | LOW confidence across Red/Yellow/Green/Blue, pending a single combined hardware-calibration pass | **Blue and Green are no longer interim community estimates — they are developer-confirmed values chosen on a real preview (D-21).** Re-tuning them during a future calibration pass would revert a locked decision, not improve a guess. Yellow and Red remain interim, uncalibrated D-13 estimates — the calibration follow-up this document originally scoped across all four inks now narrows to just those two. |
| "Illustration-zone spatial exception" (Pitfall 1, Pattern 4's addendum draft, the two-part guard-rail redesign) | A spatially-scoped guard: full 6-colour palette inside the illustration's own bounding box, `{bg_idx, IDX_WHITE}` exactly everywhere else | Superseded by the simpler `_assert_legal_palette()` — legality-plus-dominance, no spatial scoping at all. A real livery may legitimately place any legal ink anywhere on the canvas; the shipped guard rail only asserts every index is legal and the background index is the plurality. |

### What survived unchanged and is still load-bearing

This addendum is not a blanket invalidation. The following findings from the original research passes remain fully valid in the shipped code:

- **Full six-colour Floyd-Steinberg quantization against the unpadded six-entry palette** (Pattern 1) — `dither.dither_to_full_panel_palette()` still builds its quantize target directly from `panel_format.PALETTE_RGB` with zero padding, exactly as verified this session.
- **The no-remap rule** — the illustration path still applies no `.point()` remap; `PALETTE_RGB`'s index order already matches the canvas's real indices.
- **The alpha hard-threshold requirement.** Note the correction path here: this document's own Pattern 2 originally advised *against* hard-thresholding the illustration's alpha mask ("a soft alpha edge... is fine"). `03-03-PLAN.md`'s own planner findings (`F-03-resolution`) directly disproved that advice during planning — a soft mask blends palette index integers, not colours, and a mask value of 128 over Blue and White measurably produced Yellow. The shipped code follows the *correction*, not this document's original Pattern 2 text: the alpha channel is hard-thresholded at `>127` to strictly binary before every paste, exactly as `load_binary_mask()` already did for the retired flat silhouette.
- **The unpadded-palette footgun** (Pitfall 2) — still a live risk if anyone ever hand-builds a differently-ordered or padded quantize palette; the shipped code avoids it by construction.
- **The text-stroke prohibition** (Pitfall 7) — `stroke_width`/`stroke_fill` still do not appear anywhere in `server/plane/render.py`; confirmed in the shipped renderer's own module docstring.
- **The non-background-index counting technique** for shape/mirroring comparison (Pitfall 6) — still the correct approach for any test asserting a full-colour illustration was actually painted, regardless of which zone geometry or mirroring rule is current.
- **The requirement that Phase 2's legibility finding (`02-05-SUMMARY.md`, "clearly legible," Inter on a flat single-index field) must not be cited to satisfy this phase's on-glass check** (Pitfall 8) — the font change from Inter through Zilla Slab to PT Serif Regular has only made this truer, not less true. The shipped render is a third structurally different font from the one that finding covered.

### Validation Architecture reconciliation

The shipped harness counts are **26** (`server/test_render.py`), **6** (`server/test_dither.py`), and **5** (`server/test_poll_loop.py`) — not the `EXPECTED_CHECK_COUNT = 25` baseline this document's Validation Architecture section named as a mechanical follow-up. The poll-loop suite is genuinely new: it covers the two-deep flight history (`previous_flight`/`previous_route`/`previous_confirmed_state`) that this research never anticipated, because the current-plus-previous two-flight layout (D-25/D-26) was decided after this document's research passes were written. **No `server/test_illustrations.py` harness exists**, despite `03-03-PLAN.md` calling for one (Task 1's own `<behavior>` block and acceptance criteria specify it) — this is a genuine open documentation/testing gap carried forward into `03-03-PLAN.md`'s own reconciliation, not a closed item this addendum should paper over.
