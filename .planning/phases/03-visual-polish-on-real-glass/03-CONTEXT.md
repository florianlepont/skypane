...
- **D-05 [informational] (unlocking decision):** evidentiary rationale, not an independent action — the actionable instructions it unlocks are D-06 through D-13 (below), which plans cite directly. The user confirmed via **SenseCraft** (Seeed's official companion app for this panel) that a personal photo they sent displayed well on the real hardware. This proves the panel itself renders dithered/photographic content well — the flat, no-anti-aliasing, no-dither rendering rule in `02-UI-SPEC.md` Revision 2 was a deliberate **Phase 2 style choice** (the "poster" look), not a hardware limitation. `02-UI-SPEC.md`'s own rendering-rule note already anticipated this: *"Reserve Floyd-Steinberg dithering... for any future photographic content only."* This phase is that future.
...
### Aircraft-type accuracy and the procedural-rendering alternative (third scope discussion, same session)

- **Raised by the user:** rather than D-09's static AI-generated-file-per-airline hand-off, build a server-side pipeline that overlays each airline's real livery colors onto the correct aircraft-type SVG shape, computed at render time — solving the "what's the exhaustive airline list" and "what about aircraft type" problems structurally instead of by hand-generating more files.
- **D-19 (scope split decision):** This is a real, compelling architecture, but it depends on an unverified prerequisite (real aircraft-type data from ADS-B — see Phase 3.1's own note) and is a materially bigger engineering undertaking (per-type SVG region/livery-mapping work) than what's already researched for this phase. **Split out as a new inserted roadmap phase, `Phase 3.1: Procedural Per-Airline Livery Rendering`**, rather than decided inline here — see `.planning/ROADMAP.md`'s Phase 3.1 section for its own goal/success criteria. This keeps Phase 3 bounded to what's already researched (D-09's static-file approach) while not losing the idea.
- **D-19 continued (Phase 3's own type-accuracy bar, now that the procedural engine is deferred):** for Phase 3's static files, aircraft-type accuracy is a **representative/plausible type per airline** (e.g. the aircraft type that airline most commonly flies at Orly), not real per-flight type detection — explicitly not blocking Phase 3 on live ADS-B aircraft-type data, which Phase 3.1 will pursue properly. The user's own words: "type plausible/représentatif par compagnie (suffisant pour Phase 3)." Update the D-09 hand-off spec's illustration-selection guidance accordingly — the user (or whoever picks the reference aircraft per airline for the illustration prompt) should pick a real, commonly-seen type for that airline at Orly, not an arbitrary/generic jet shape, but the render pipeline itself does not need to branch on a live-detected type field this phase.

### Post-execution course-correction after reviewing real render previews (fourth scope discussion, same session)

After Waves 1-2 executed (D-15 typography as Zilla Slab, D-17 dithered mood-gradient background), the user asked why no visual preview was shown before execution — a fair process gap. In response, real preview PNGs were generated directly from the actual render pipeline (`server/plane/render.py --preview`, and a couple of in-memory monkeypatched variants for quick comparison, no repo files touched) and shown to the user before Waves 3-4 proceeded further. Reviewing them produced two corrections:

- **D-20 (typography, supersedes D-15's Zilla Slab pick):** The user didn't like Zilla Slab's look in practice. Four real candidates were rendered side by side (Zilla Slab, Bitter, Source Serif 4, PT Serif — all SIL OFL 1.1, all with robust Bold/SemiBold weights, no thin hairline cuts, matching the same e-ink legibility constraint D-15 already established). **User's choice: PT Serif.** `PTSerif-Bold` was fetched this session (static weights, not a variable font) from Google Fonts' `google/fonts` repo — needs the same pinned-commit + sha256 + OFL-text vendoring discipline `ZillaSlab-*`/`Inter-*` already got in `server/assets/fonts/VENDOR.md`, and PT Serif also needs a Regular/SemiBold-equivalent weight sourced for the Caption/Destination roles (only Bold was fetched for the quick comparison) — left to implementation to source the right weight file(s) from the same family.
- **D-21 (background, supersedes D-17's dithered mood-gradient):** After seeing the actual dithered/speckled result on a preview, the user did not want the textured/gradient look after all — despite D-17 explicitly anticipating and accepting the "photographic-toned, dithered" direction in the abstract. **User's choice: revert to a flat, solid single-color background per state** (closer to Phase 2's original locked contract), but with a **new, lighter color** — not the D-13 interim "muted ink" Blue `(80, 128, 184)`, which the user found too dark. A lighter sky-blue `(110, 180, 225)` was previewed and **confirmed as "parfait."** The equivalent lighter tone for the Green (arriving) state was not yet previewed or confirmed — needs the same before/after comparison treatment before implementation. **Practical consequence:** this substantially reverts Wave 2's (03-02) core contribution (`server/plane/dither.py`'s `build_mood_background()`) back toward a flat `pf.new_canvas(bg_idx)`-style fill — D-18's "two dithered moods" state-signal mechanism becomes moot (a flat single legal color per state already unambiguously signals state, exactly as Phase 2's original design did). The quiet-zone rectangle pattern (`draw_quiet_zone()`) may no longer be strictly necessary once the background is flat again (flat-text-on-flat-color doesn't need a legibility backing the way dithered content does) — but keeping it is low-risk/harmless and avoids more rework; leave it in place unless it visibly causes a problem. **This does not touch D-11/D-12/D-13's illustration-zone multi-color exception (Wave 3's job) — only the background field itself.**

**Sequencing note:** these two corrections were identified while Wave 3 (03-03, illustration work) was already executing in the background on the shared main working tree (no worktree isolation this run). To avoid concurrent-edit conflicts, D-20/D-21 are recorded here as decisions but their actual code changes are deferred until Wave 3 completes or reaches its own checkpoint.
