...
- **D-03:** **A dedicated, sober off screen** — the same shape as Phase 10's quiet-hours state (`_build_quiet_hours_canvas()`), but **without any return-time promise**, since a manual toggle has no "Back at HH:MM" it can honestly make. A short heading plus one explanatory line indicating the frame was switched off from the interface.
  ...
  **D-03 revised on glass — 12-06 session, 2026-09-05.** The "same shape as quiet hours" half of this decision is SUPERSEDED; the "dedicated, sober, no return-time promise" half stands. 12-02 shipped D-03 as written, the developer read it on the real panel and asked for something more elegant, and after five sketched candidates chose a **dimmed composition**: the Grey theme's dithered dark field, white ink, a glyph above a tracked Bold label over a short rule, then the body. Seeing it on glass, the developer then asked for the quiet-hours screen to be reworked the same way. Both now draw through one shared routine (`_build_dimmed_hold_canvas()`), and the glyph — power ring versus crescent — is what tells them apart. The blank-field rejection above is unaffected: this is not a blank field, it is a dark one with a full composition on it, and it cannot be mistaken for a dead device.

  *What this changes in the model.* The hold screens become a system: **dark means the frame is resting on purpose** (off, quiet hours), **white means it is working** (empty state, flight boards). The distinguishability question the UI-SPEC flagged was thereby answered by construction, and the developer validated every named point on real Spectra 6 ink: the field reads as an even grey (not the preview's noise), Regular white body text holds at 40px, and the Bold-class label, rule and glyphs are crisp. This was a choice made on glass at the developer's request, not the UI-SPEC's escalation ladder — recorded as such in `12-UI-SPEC.md`'s revision block and in `10-UI-SPEC.md`'s amendment note.

  *A moon was considered and placed correctly.* The developer's first instinct for the off screen was a "sleep" glyph; a moon there would have pulled DISPLAY OFF toward the curfew screen's night register, so the power ring went on DISPLAY OFF — and the crescent went where it belongs, on QUIET HOURS.

  *The empty state followed, on its white field.* Seeing the empty screen right after the two dimmed ones, the developer asked for it to get the same treatment and a mark of its own. It now draws through the same routine (`_build_hold_canvas()`, of which the dimmed pair is one variant) as the white variant — a runway glyph (strip, threshold "piano keys", dashed centreline), the runway-dependent heading as a tracked label, the rule, the body — so all three hold screens are one family and **the field alone says which kind of screen it is**. Two copy-level adjustments came from the glass: the body breaks before "The display" rather than by width (which had orphaned "one is."), and the Phase 2 em dash was retired for two plain sentences — the session's only change to locked copy. The developer validated the empty screen without a further glass cycle once the dash was gone.
