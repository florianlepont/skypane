...
Register consolidated from the `<threat_model>` blocks of `15-01-PLAN.md` through
`15-05-PLAN.md` (15 distinct threat IDs, several appearing in more than one plan).
...
| T-15-04 | Denial of Service | registry growth | medium | mitigate | `COLOUR_RULE_MAX_ENTRIES = 200` (`colour_rules.py:58`); hard reject on a new key at the cap, replace explicitly exempt (`:328-331`); bounded read stops accumulating at the cap (`:253-255`); route surfaces it as the registry-full flash (`app.py:1621-1622`) | closed |
| T-15-05 | Tampering | theme-id membership, three gates | high | mitigate | `normalise_rule_theme_id()` (`colour_rules.py:195-204`) applied before persisting (`:322`), on every read (`:263`), and the resolver re-checks membership on **every** return path before handing a value back (`:498`, `:503`, `:507`, falling back to `DEFAULT_THEME_ID` at `:509`). 15-03's `transfer` of this row to 15-01 is honoured: no second gate exists at the `build_canvas()` call sites, and none is needed | closed |
| T-15-06 | Tampering | `theme_arriving` write validation | high | mitigate | Two independent lines, both present: `config_page.handle_post()` membership test against `device_config.THEME_IDS` before the value leaves the HTTP layer (`companion/pages/config_page.py:1546-1550` → `FLASH_SAVE_FAILED`, nothing written), and `save_device_config()`'s own `ValueError` before the file is touched (`server/device_config.py:663-664`) | closed |
| T-15-07 | Denial of Service | corrupt-file never-raises contract | low | mitigate | `load_colour_rules()` catches `OSError`/`ValueError` and coerces a non-dict top level (`colour_rules.py:232-238`), guards every per-entry shape (`:260-270`); `resolve_effective_theme_id()` tolerates a non-dict cache, `None`/non-dict flight, and a `theme`-less `device_cfg` (`:480-509`); `load_device_config()` keeps the same contract with `theme_arriving` joining the normalise-on-read discipline (`device_config.py:589-606`) | closed |
| T-15-08 | Tampering | `theme_arriving` read validation | medium | mitigate | `normalise_theme_arriving()` degrades a non-member to `None` — never to `DEFAULT_THEME_ID` (`device_config.py:456-476`), wired into the read path at `:598`. Independent of the write-path gate (defence in depth behind T-15-06) | closed |
| T-15-09 | Tampering | `None`-versus-sentinel write distinction | medium | mitigate | `CLEAR_THEME_ARRIVING` is a distinct module-level `object()` (`device_config.py:111`) compared **by identity** (`:694`); `None` keeps its single meaning "carry forward" (`:699`). The HTTP layer passes the sentinel, never `None`, on the clear path (`config_page.py:1565`) | closed |
...
| T-15-11 | Tampering | `theme_arriving_enabled` checkbox branch | medium | mitigate | Explicit three-way branch: absent → clear, exact `ARRIVING_CHECKBOX_VALUE` → on, anything else → reject the whole submission (`config_page.py:1564-1569`), matching the three pre-existing checkboxes | closed |
| T-15-12 | Denial of Service (of function) | the CSS-only arrivals reveal | low | accept | Acceptance rationale re-verified in shipped code — see Accepted Risks Log ACC-15-01 | closed (accepted) |
| T-15-13 | Cross-site request forgery | the two new state-changing POST routes | medium | transfer | Transfer target verified to actually cover both new routes — see Transfer Verification below | closed |
| T-15-14 | Information disclosure / injection | `rule=` echoed into a flash message | medium | mitigate | Validate-then-display in `_resolve_flash_text()` (`companion/app.py:460-464`): the value is re-normalised through `normalise_rule_callsign()` (charset `[A-Z0-9]{2,8}`, a strict superset of the hex and prefix charsets) and falls back to the generic added copy on failure; `escape_html()` in `layout.flash_banner()` (`companion/layout.py:1044`) is the second line | closed |
| T-15-SC | Tampering (package installs) | package installs | n/a | accept | Acceptance rationale re-verified against the diff — see Accepted Risks Log ACC-15-02 | closed (accepted) |
...
|------|---------|----------|------------|
| UF-15-01 | `save_device_config()`'s unguarded read-modify-write (`server/device_config.py:687-725`) now carries this phase's new `theme_arriving` field | medium | **Same threat class as T-15-02 (lost update), applied to a store no T-15-02 register row names.** T-15-02's declared mitigation site is `colour_rules.py`'s mutators (15-01) and the per-cycle priming call (15-03); `device_config.json`'s save path is outside every plan's stated scope. The gap is pre-existing (the file had no lock before Phase 15 either — confirmed independently by 15-REVIEW.md WR-02), so this phase widened an existing unguarded surface by one field rather than creating one. Two concurrent `POST /settings` requests can lose an update; no validation is bypassed and no crafted value can be persisted. Already filed as WR-02 follow-up work. |
| UF-15-02 | `colour_rules.rule_rows()` (`colour_rules.py:401-424`) and `_rule_theme_swatch_html()` (`companion/pages/config_page.py:1130-1140`) are not themselves allowlist gates | low | `rule_rows()` re-checks only `isinstance(str)`, and the swatch helper does an unguarded `device_config.THEMES[theme_id]` lookup. Both are safe **today** solely because their only production caller feeds them a registry that already passed `load_colour_rules()`'s membership gate (`companion/app.py:996` → `config_page.py:1258-1262`). Verified: a hand-edited file with an unregistered `theme_id` is dropped on read before it can reach the swatch, so no `KeyError`/500 path exists. Noted as a single-gate dependency a future caller could break, not as an open threat — T-15-05's declared read-side gate is present and is what protects it. |

---

## Residual Notes on Closed Threats

- **T-15-02 / WR-01.** `_handle_rule_delete()` (`companion/app.py:1654-1661`) computes
  ...
  *reporting* inaccuracy at the HTTP tier, not the lost-update tampering T-15-02
  declares and mitigates, and 15-05 explicitly dispositioned the HTTP tier as
  `transfer`-to-15-01 rather than adding a second locking layer. **T-15-02 stays CLOSED**;
  the flash-accuracy race is tracked as WR-01 follow-up work.
- **T-15-06.** A non-string, unhashable `theme_arriving` (e.g. a list) reaching
  ...
  check's shape — not a Phase 15 regression.
...
- **Executed proofs, not just located them.** `server/test_colour_rules.py` (27/27),
  ...
  20-thread concurrent-writer check (T-15-02), the hostile-input write+read sweep
  (T-15-01), the tampered-cache check (T-15-05), the byte-identical-file rejection
  checks (T-15-06), the crafted-checkbox rejection (T-15-11) and the
  unauthenticated-POST-writes-nothing check (T-15-13).
- **Behavioural spot checks against the live modules** (read-only, via
  ...
  generic added copy, only `AFR1234`/`afr1234` reached interpolation (T-15-14);
  `resolve_effective_theme_id()` with a tampered cache entry carrying
  `theme_id="../../etc/passwd"` returned the base theme, not the crafted id (T-15-05);
...
- **Call-site counts re-derived from the shipped file**, not from the summary's claim:
  ...
  `grep -c "theme_id=effective_theme_id" server/poll_loop.py` == 2 (T-15-03).
...
- [x] Accepted risks documented in Accepted Risks Log (ACC-15-01, ACC-15-02)
