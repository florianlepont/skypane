...
- [x] Device wakes on a schedule, polls the server over HTTPS, downloads a new image if one is available, displays it, then returns to deep sleep (mirrors flightportrait's wake/poll/backoff model) (Phase 1, 01-01..01-07 on real hardware against the stub server, 2026-08-26; against the real OVH deployment since 02-05; the wake interval became web-configurable in Phase 11 and the off-state cadence was pinned by Phase 12 — reconciled here 2026-09-06, the Active → Validated move had never been replayed)
- [x] User can see a low-battery indicator on the frame when the battery is running low (Phase 5, DEVICE-04 — 05-02 server half, 05-03 device half with the calibrated ADC read, icon confirmed on real glass 2026-08-27; reconciled here 2026-09-06)
- [x] Device runs on battery power for v1 (no wall power, no solar) (in daily use on battery since the Phase 1 flash, telemetry `X-Battery-Mv` on every poll; the *measured* multi-day viability is DEVICE-05, still open — this line validates the constraint, not the measurement; reconciled here 2026-09-06)
- [x] Server (small cloud VPS) ingests public ADS-B aggregator data for runway detection, renders display images, and serves them via a poll protocol the device calls (Phase 2 end to end on the OVH VPS-1 since 2026-08-26, Phase 4's gated deploy since then; the "fetches RER data" clause of the original line is v2 and moved to Active below — reconciled here 2026-09-06)
...
- [ ] (v2/later) Server fetches RER data for the Orly-Ville view (the RER clause split out of the original server requirement on 2026-09-06; the ADS-B/render/serve half is Validated above)
...
| The companion may create illustration/identity keys at runtime, in a `state_dir` registry, instead of only editing the git-tracked `_ICAO_AIRLINE_PREFIXES` / `_ILLUSTRATION_TARGETS` tables | Closing a coverage gap from the web UI is impossible otherwise: those two tables are coupled by a test drift guard and only change through a commit and a redeploy. The runtime registry is consulted *after* the static table, so the curated source stays authoritative and a later upstream correction wins (with the superseded manual entry flagged, never silently dropped). This is the project's first runtime-mutable identity state — previously `illustration_overrides/` could only replace the pixels of an already-known key, never mint one | Shipped Phase 13, verified 14/14 must-haves (`13-VERIFICATION.md`) |
| `enrich.airline_from_callsign()` gives up its documented purity (pure, no I/O, returns only fixed-table values) so it can consult that registry | Unavoidable consequence of the decision above. The safety invariant did not disappear — it moved upstream to `manual_resolutions.add_entry()`'s positive slug allowlist, re-applied on every read, with a raw-name traversal guard in front of it (`normalise_airline_key()` is total and would otherwise reduce `../../etc/passwd` to the innocuous-looking slug `etc-passwd`). The downstream path-construction boundaries now carry the weight alone and were re-verified as sufficient | Shipped Phase 13; threat `T-hyy-01` re-argued rather than deleted, and `T-v26-02-01`'s validate-then-join property re-established over the widened membership set |
| CI test suite: split `companion/test_browser_ux.py` (96 checks) into a shared helper module plus 2 standalone harnesses (11 + 9 checks), joining the existing `scripts/run_all_tests.py` worker pool; stopped after 2 of the allowed 2-3 extractions rather than taking the optional 3rd (Flights, 8 checks) | Measured, not assumed: the ~30-40% CI-job wall-time gate was missed on every one of 5 local samples (9.1%-29.6%) AND on the real CI run that followed (PR #78: 3.8%-8.6%). Root cause identified and confirmed on real CI: 3 Chromium-launching harnesses now compete for the pool's 4 worker slots (only 1 did before), slowing every other harness 22-77% locally and similarly on the real 4-vCPU runner — a 4th concurrent Chromium (Flights) would very likely add contention rather than relieve it | Shipped Phase 31 (5/5 plans), verified 9/10 must-haves programmatically + 2/2 human (real CI: 24/24 harnesses green, no new flakiness). Explicit recommendation against opening a follow-up settings-mega-cluster decomposition phase under the current pool mechanism — `31-TIMINGS.md` has the full arithmetic |
