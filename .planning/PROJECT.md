...
- **Plane detection approach**: free public ADS-B aggregator APIs (airplanes.live primary, adsb.fi secondary), geofenced to runway 3's flight path, detect real aircraft directly (one plane at a time, departure or arrival depending on current wind-driven runway configuration), rather than pulling a schedule from a public flight-data API. Originally planned around a local RTL-SDR receiver; Phase 1 plan 01-04 validated the aggregators clear the coverage bar (~92min real traffic, 38/37 distinct aircraft, 2/2 on-ground) with no RTL-SDR hardware needed — corrected here 2026-08-26 at Phase 1 close, see Key Decisions.
...
---
*Last updated: 2026-08-27 after Phase 3 completion (gap closure: render-path illustration decode guard)*
