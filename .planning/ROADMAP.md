...
- [ ] 28-01-PLAN.md — the gear icon: one symbol appended to the sprite, one `icon_html()` argument changed, the hard-coded `ICON_IDS` count bumped 22 -> 23, and a check that the panel still has no page-nav links so the unchanged label stays honest (CFG-76)
- [ ] 28-02-PLAN.md — the dial's Bug B: `button:active` excluded from `.value-control__handle`, `transform` dropped from the handle's transition, and THE check that SAMPLES the handle's radius throughout a held press — with the endpoint-only version mutation-proven to pass on broken code (CFG-73)
