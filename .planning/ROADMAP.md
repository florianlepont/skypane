...
### Phase 14: Per-direction themes, per-flight colour rules and roster-linked highlighting
...
**Discussed 2026-09-06 — everything below this block is the PRE-DISCUSSION framing and is retained only as a record.** `.planning/phases/14-per-direction-themes-per-flight-colour-rules-and-roster-link/14-CONTEXT.md` is authoritative; read it first and treat it as superseding this section wherever the two disagree. What the discussion settled:

1. **The split is (b).** Phase 14 ships the resolution seam, the per-direction theme and the per-flight rules; **the roster half is deferred** — it stays in SEED-003 with the developer's intent recorded in the CONTEXT's `<deferred>` section (iCal subscription URL; theme-only rendering; flight number + day matched via `callsign_iata`; the URL as an environment-variable secret), to be promoted as its own phase once the export format and K Stewart's consent are in hand. Nothing anticipatory is built for it (D-02).
...
**The central design decision — the split.** Three options, to be chosen first in `/gsd-discuss-phase 14`; do not assume a default:
...
- **(b) Phase 14 = the resolution seam + sub-ideas 1 and 2; the roster half stays in SEED-003 as the explicitly deferred third step**, re-promoted as its own phase once the export format and consent are in hand. Everything in (b) is decidable by the developer alone today, and the rule store it builds is exactly what a roster match would write into later — a roster match *is* a rule, sourced automatically instead of typed. **This is the recommended split.**
...
- [x] 14-05-PLAN.md — Settings rules editor: add form, list with per-row delete, and the two immediate POST routes outside the dirty bar (D-10/D-11); carries the closing no-JS browser check [wave 3]
