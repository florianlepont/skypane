...
**Goal:** The developer tested Phase 27's deployed fixes hands-on and reported five more findings. Each was independently investigated against the live app (rendered pages, driven interactions, traced wiring) before this phase was scoped — one did not reproduce, one is confirmed but was left as-is at the developer's own written choice pending a design decision that has now been made, and three are confirmed defects with named root causes. This phase fixes the four real items. It adds no new capability beyond the carousel's decided behavior.
...
- Fix item 2: the interactive quiet-hours readout must format both endpoints as HH:MM and compute a live duration on every interaction, matching the server-rendered initial format exactly rather than degrading to raw minutes with a blank duration.
- Item 3: no implementation work. A plan may add a regression-proof check that the runway radios' `form=`-attribute auto-save path (a distinct wiring path from nested controls) is covered, since investigation found no existing check specifically proves it.
...
**Carried discipline from Phase 27:** every new check must assert relationships, not just endpoints (item 2's fix is proven by the readout's actual displayed text matching computed HH:MM + duration after an interaction, not by the existence of a format function; item 4's fix is proven by the preview `<img src>` actually matching whichever chip is geometrically centered during a scroll, not by a scroll listener being attached). Mutation-test every new check, with the failure message quoted. `git checkout-index -f --` for any mutation revert, never `git checkout --`. `EXPECTED_CHECK_COUNT` re-derived by running.
