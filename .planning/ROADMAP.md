...
- [x] **Phase 18: Companion audit & UX refactor** - A full audit of the companion (every page, dialog and state screenshotted at desktop and phone width, plus three code reviews), motivated by a second household member with basic computer skills using it: navigation regrouped into everyday (Home, Display, Flights, Airlines) and Advanced (Health, Device) tabs, a new Home page of widgets with one-tap screen/quiet-hours/refresh actions, Settings split into Display and Device per a new screen-type registry (`companion/screens.py`, the seam for future screen kinds), Paris local time everywhere, and 18 of 46 consolidated findings fixed in the same pass with the rest tracked in `18-AUDIT.md`. (completed 2026-09-10)
...
### Phase 15: Per-direction themes, per-flight colour rules and roster-linked highlighting

**Goal:** Something more specific than "the one active theme" can decide what the frame looks like for a given render, at three escalating levels of automation: the operator picks the panel theme per direction (departures vs arrivals) instead of one theme for both; a per-flight rule keyed on a callsign or an ICAO24 hex overrides the look whenever that exact flight or airframe is detected; and a flight K Stewart is working as crew, read from their duty roster, is highlighted automatically — turning the frame from a generic runway board into a "someone I care about is flying right now" signal. All three share one seam: a resolution step ahead of the existing theme lookup in `poll_loop.run_once()`, which today reads one `theme_id` per cycle and hands it to every `build_canvas()` call unchanged.

**Discussed 2026-09-06 — everything below this block is the PRE-DISCUSSION framing and is retained only as a record.** `.planning/phases/15-per-direction-themes-per-flight-colour-rules-and-roster-link/15-CONTEXT.md` is authoritative; read it first and treat it as superseding this section wherever the two disagree. What the discussion settled:

1. **The split is (b).** Phase 15 ships the resolution seam, the per-direction theme and the per-flight rules; **the roster half is deferred** — it stays in SEED-003 with the developer's intent recorded in the CONTEXT's `<deferred>` section (iCal subscription URL; theme-only rendering; flight number + day matched via `callsign_iata`; the URL as an environment-variable secret), to be promoted as its own phase once the export format and K Stewart's consent are in hand. Nothing anticipatory is built for it (D-02).
2. **Per-direction theme = one theme plus an optional arrivals override** (D-04), never two symmetric fields; the existing `device_config.json` stays valid with no migration. In Settings, a checkbox under the Theme group reveals a second, identical chip grid (D-05).
3. **Rules resolve to registered theme ids** (D-07), keyed on an exact callsign, an ICAO24 hex, or a 3-letter prefix (D-08) with a fixed most-specific-wins order and one rule per key (D-09); the editor lives on Settings under the Theme group with a native `<select>` for the target theme and immediate add/delete routes outside the main form (D-10/D-11); the store is a `state_dir` JSON copying `manual_resolutions.py`'s contract (D-12); one resolver applied to both render branches (D-13).

**Closes with (revised):** no on-glass verification — nothing new reaches the glass by construction; a `/gsd-secure-phase 15` pass over the rules routes and store. The roster-half security flags below now belong to the future phase.

**Note on origin (2026-09-06):** Promoted from `.planning/seeds/SEED-003-theme-direction-scope-color-rules-calendar-highlighting.md` at the developer's request ("Lancement SEED 3"), the last dormant seed with a real design conversation behind it — planted mid-conversation on 2026-09-02 from the developer's own description of the three ideas, and enriched on 2026-09-04 with the named crew case. The seed scoped it *large* and says plainly it is three features, each phase-sized, deliberately kept in one file because they share the resolution seam and are worth reviewing together — "but they don't have to ship together". This entry keeps that framing: the phase is promoted whole so that `/gsd-discuss-phase 15` reviews all three against each other, and **the first thing to settle there is the split** (below). The three remaining dormant seeds (AeroDataBox destination lookup, presence-adaptive poll cadence, and the local RTL-SDR backup already recorded as not feasible) were not in contention.

**What the code says today, checked before promotion — three corrections to the seed.**

1. **The per-theme departing/arriving colour pair the seed wants to "reconcile" no longer carries any information.** Every one of the 18 registered themes has `departing_index == arriving_index` (verified by iterating `THEMES`): Phase 8 made every colour theme a single flat colour by decision (`08-CONTEXT.md` D-02) and then retired "Sky", the one two-tone Blue-departing/Green-arriving theme, outright on the developer's explicit instruction (`08-06-SUMMARY.md`; `device_config.py`'s own `THEMES` comment). The seed's central design question for sub-idea 1 — does per-direction scoping stack on top of the existing pair mechanism, or replace it? — therefore has a plain answer: **there is nothing to stack on.** Sub-idea 1 reduces to a second theme *selection*: `device_config.json` grows from one `"theme"` to a departures theme and an arrivals theme (exact shape — two full fields, or one theme plus an optional arrivals override — to settle), `theme_background_index(state, theme_id)`'s `state` argument becomes the thing that picks *which theme id* rather than which index inside one theme, and the Settings page's theme chip grid (`theme_fieldset()`, `config_page.py:241` — 18 chips, each with a rendered preview) is either doubled or given a direction switch, a control-density question `sketch-findings-skypane` must weigh in on. Discuss-phase should confirm this is wanted knowing the history: the developer chose single-colour themes *over* paired departing/arriving looks in Phase 8; per-direction selection re-introduces direction-dependent looks at the selection level rather than the registry level, which is a different thing, but it is the same visual outcome "Sky" produced.

2. **A rule should resolve to a theme id, not to a colour.** The seed phrases sub-idea 2 as "override the display color". Every ink/weight/dither/band combination on this project was confirmed on real Spectra 6 glass one theme at a time (Phases 8 and 9, quick task 260905-e04), and `draw_main_text_block()` takes ink and weight from the theme, never from a bare colour index. A rule carrying a raw colour would mint untested combinations and pull an on-glass verification into every rule; a rule naming one of the 18 registered themes reuses everything already verified and needs no new render code — the render path already takes `theme_id` as a parameter at every `build_canvas()` call site. This should be the default framing unless the developer wants something no theme can express.

3. **"Tail number" is not available to match on today, but the hex is.** `detect._normalise_selection()` carries `hex` (the ICAO24 address — the airframe identity the seed is actually after) and `callsign`; it does not carry the aggregators' registration field (`r`), so a rule keyed on a registration such as F-HBNA needs either a one-line addition to the selection dict or a hex-only matcher. The callsign side is settled: rules key off `enrich.normalise_callsign()` output, exactly as the seed says.

**The central design decision — the split.** Three options, to be chosen first in `/gsd-discuss-phase 15`; do not assume a default:

- **(a) One phase, all three sub-ideas**, planned in waves (seam + per-direction themes → rules → roster). Largest, and the roster half cannot be planned until two questions only K Stewart can answer are answered (below) — so (a) risks a phase that cannot close.
- **(b) Phase 15 = the resolution seam + sub-ideas 1 and 2; the roster half stays in SEED-003 as the explicitly deferred third step**, re-promoted as its own phase once the export format and consent are in hand. Everything in (b) is decidable by the developer alone today, and the rule store it builds is exactly what a roster match would write into later — a roster match *is* a rule, sourced automatically instead of typed. **This is the recommended split.**
- **(c) Three phases, one per sub-idea.** Cleanest bookkeeping, but sub-ideas 1 and 2 share the seam and the Settings-page work closely enough that separating them would build the seam twice.

**The roster half — two external unknowns and one prerequisite, none of which discuss-phase can settle on its own.** (i) *Export format*: does K Stewart's crew app expose an iCal subscription URL, and does a duty event carry a plain flight number, an internal duty code, or both? This decides the parser and is the single blocking unknown. (ii) *Match key*: a roster names the scheduled IATA flight number ("AF1234"); the frame detects an ICAO callsign ("AFR1234"), and for the rotating-callsign carriers this project already fights (`aerodatabox-destination-lookup-rotating-callsigns.md`) the two may not line up. `route["callsign_iata"]` (adsbdb, Phase 8 D-09) is the bridge to check first — which means roster matching must run **after** `enrich.resolve_route()`, not on the raw detection. (iii) *Consent*: this stores a named person's work schedule on the VPS. Their explicit agreement is a prerequisite, not a courtesy.

**Also to settle in discussion:** (a) **precedence** — a per-flight rule must beat the per-direction theme, and a roster match must beat or equal a manual rule; the resolution step must be one function with one documented order, not something emergent from call sites; (b) **where the override is applied** — `run_once()` reads `theme_id` once at the top of the cycle (`poll_loop.py:742`), but the flight and its route are only known after enrichment (`:1057`), and the source-fault/battery re-render branch (`:1188`) draws the *same* flight again from `current_route` — an override applied in one branch and not the other would flip the panel's colour on a battery-icon repaint; (c) **the rule store's home** — a dedicated `state_dir` JSON following `server/plane/manual_resolutions.py`'s contract exactly (never-raising load, `normalise_*` gates, tmp-write-then-`os.replace()`, bounded entry count) rather than a growing list inside `device_config.json`'s eight bounded scalars, mirroring Phase 13's D-05 reasoning; (d) **the editor's home** — Settings, beside the theme picker, or a new section on Airlines beside Phase 13's manual-resolutions list, this codebase's only add/delete list precedent; (e) **the fetch cadence** for a roster feed, given the server has no long-running poll process — `deploy/skypane-poll.timer` fires a 30 s oneshot — so a throttled fetch inside the oneshot with its result persisted to `state_dir` is the natural shape, and it must never delay the render path on a slow upstream.

**Security, flagged now for `/gsd-secure-phase 15`.** The roster URL would be **the project's first runtime-stored secret**: crew-app iCal URLs embed a private token, `device_config.json` is documented as holding no secret (its module docstring makes that promise), and the companion's one existing secret lives in the process environment (`auth.PASSWORD_ENV_VAR`), never on disk. It must never reach a log line, the Health page, the gallery, or a flash message. Fetching an operator-supplied URL from the VPS is also new outbound egress under operator control — HTTPS only, no private address ranges, bounded response size and timeout, the same class of gate as the upload path's `MAX_ILLUSTRATION_UPLOAD_BYTES`. Callsign/hex rule keys are free-typed operator input reaching a registry consulted on every cycle — the same threat shape `T-13-02` answered with a positive allowlist regex.

**Expected surface** (if (b) is chosen; to be confirmed at plan time): one new resolution function in `server/device_config.py` or a sibling leaf module (the theme for this render, given state + flight + route + config + rules); a second theme field in `device_config.json` with the same never-raising normaliser and `save_device_config()` carry-forward; a rule registry file in `state_dir` mirroring `manual_resolutions.py`; the Settings theme group extended per direction plus a rules editor in `companion/pages/config_page.py`; one hook in `poll_loop.run_once()` applied consistently to both render branches; `companion/theme_preview.py` untouched (previews are per theme id, which rules reuse as-is).
...
**Closes with:** no on-glass verification if rules resolve to existing theme ids (nothing new reaches the glass — to be confirmed once the split and the rule shape are settled); a `/gsd-secure-phase 15` pass in any case, and a blocking one before the roster half ever ships.
**Plans:** 5/5 plans complete

Plans:
**Wave 1**

- [x] 15-01-PLAN.md — `server/plane/colour_rules.py`: the rules registry (D-08/D-09/D-12) and the D-13 effective-theme resolver, plus the new `server/test_colour_rules.py` harness and its suite registration (both Wave 0 items) [wave 1]
- [x] 15-02-PLAN.md — `server/device_config.py`: the optional `theme_arriving` key, its degrade-to-`None` normaliser and the `CLEAR_THEME_ARRIVING` sentinel that makes it genuinely clearable (D-04) [wave 1]

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 15-03-PLAN.md — `server/poll_loop.py`: one cache-priming call plus the resolver at exactly the two `build_canvas()` call sites that display a flight, leaving the other four on the bare base theme (D-13/D-06) [wave 2]
- [x] 15-04-PLAN.md — Settings Theme group: the arrivals checkbox, the second chip grid, the CSS-only `:has()` reveal and the unchecked-clears-the-override save path (D-05) [wave 2]
...
- [x] 21-04: Frame strip, nav state reminder, Home rebuilt (wave 3)
- [x] 21-05: One "Frame colours" view and the empty-string clear signal (wave 4)
...
### Phase 22: Companion audit round 4: fix the blocking Display save bar, contradictory frame-state signals, duplicate screen and quiet-hours controls, UTC and i18n leaks and layout defects
...
- [x] 22-04-PLAN.md — X2 consumers: the Frame strip and Health's Frame tile speak one state; B13/C2/C6/T9 and the one time-value role (wave 3)
- [x] 22-05-PLAN.md — X1/D-04: one control per setting, one computed delay sentence, and the two silent-regression guards (wave 3)
...
- [x] 22-10-PLAN.md — Display and Device geometry: X6, B9, B14, B15, B7/C3, B6, B8, B17, T10, T12, C1's legend (wave 6)
...
### Phase 28: Companion review feedback round 2 — five more findings from the deployed app, investigated before planning
...
1. *"Ok mais visuellement les titres sont toujours incohérents !"* — after 27-06's investigation found no structural defect in the title markup, the developer is reporting the same complaint again on the deployed app. Investigated fresh, this time by rendering both settings pages and reading `getComputedStyle` rather than grepping markup: Device-page cards (Diagnostic LED, Wake interval, Notifications) were never wrapped in the "nested supersection" style Display-page cards already use, so they render at 22px/weight 400/serif (the shared `.text-heading` default) while Display's nested cards render at 16px/weight 600/sans (`companion/static/style.css:5703-5705`). 27-06 measured this exact delta and called it an intentional "third state" without ever looking at both pages side by side — the visual result is two unrelated typographic registers for the same semantic element depending only on which page you're on. **This is a real defect Phase 27 missed by checking markup instead of rendering.**

2. *"Le curseur des heures calmes fonctionne mieux mais est toujours bugué"* — the cross-surface agreement bug 27-02 fixed (arc/caption not following the handles) is confirmed still fixed under drag, keyboard, typed-field and preset interaction, including midnight-wraparound. But the caption's readout FORMAT regresses on the first interaction and never recovers without a full page reload: `"23:00 → 07:00 · 8h"` becomes `"300 → 420 · "` (raw unconverted minutes, permanently blank duration) after any drag, key, typed edit or preset click. This is 27-02's own documented "known limitation" (`companion/pages/config_page.py:3050-3099`, `quiet_dial_readout_html()`) surfacing as exactly what a developer calls "still buggy." **A second, distinct dial bug surfaced on follow-up:** *"Le bug est aussi que le curseur se déplace au milieu de l'horloge"* — the handle collapses toward the dial's centre during any press held longer than a couple of frames, not just a drag. Diagnosed and CONFIRMED with an exact repro: `.quiet-dial__handle` is a real `<button>`, and the global `button:active { transform: translateY(1px); }` rule (`companion/static/style.css:2624-2626`) wholesale-replaces the handle's own ring-positioning transform for the duration of the press, animated by the shared `transition: transform .15s ease` (`companion/static/style.css:2559-2583`) — measured live: 78px from centre (correct, on the ring) at press, collapsing to 14-16px (the centre) within ~90-150ms, recovering ~200ms after release. **Correction, found during 28-02's execution 2026-09-15:** the claim just above ("same shared base class affects the wake-interval slider's handle too") is WRONG — `.value-control__handle` has exactly one consumer in the codebase (the quiet-dial handles, `companion/pages/config_page.py:2980`); the wake-interval slider is a native `<input type="range" class="wake-slider__input">` whose own pre-existing CSS comment states it deliberately does not wear `.value-control`, so `button:active` cannot reach its thumb at all. No wake-side edit was made or needed; 28-02's fix is scoped to the dial alone, at the shared-class level, so any future `.value-control__handle` consumer is covered by construction.

3. *"Quand je fais un changement de paramètre (comme la piste) je ne vois pas le bouton enregistrer apparaître"* — first investigated as a single-control question and **not reproduced** in Chromium (the "Saved" status appeared correctly for the runway radio via its `form=`-attribute wiring). The developer's follow-up escalated this completely: on a real iPhone in Safari, **nothing saves at all** — not the runway, nothing — with no error shown. On Mac Safari, the developer then tested every setting on the page and got the same result: no save confirmation, no error toast, for anything. A dedicated deep-dive (reading `dirty-state.js` end to end, the toast element's CSS, script load order and every fetch/CSP option in use) found no WebKit-incompatible API, syntax, or obvious CSS collision — every failure branch in the save code does call the toast function, and the toast itself is `position: fixed` with no transformed ancestor found. **The exact root cause could not be confirmed without a live, tethered WebKit debugger**, which isn't available in this project's development environment, and the developer isn't able to run one for the next several hours. This is now the most severe item in the phase: Phase 27 replaced the settings form's manual save button with auto-save-only, and if nothing currently saves in Safari, that is a real, live regression for any Safari user (which very plausibly means the developer's own primary device). **Decision, taken with the developer 2026-09-15: build resilience rather than guess at the unconfirmed cause** — see CFG-74's scope below.

4. *"Change pictures ne s'affiche pas quand je scroll ce qui n'est pas dingue d'un point de vue UX"* — confirmed as coded, by design: the carousel's dots are decorative per-theme-colour swatches carrying no position state at all, and the live preview `<img>` only ever changes on an actual chip click/keyboard-select, never on scroll (`companion/pages/config_page.py:1594-1607`). **Product decision made 2026-09-15 (AskUserQuestion, developer's choice):** while scrolling/swiping, the live preview image should update to follow whichever chip is currently centered in the strip, WITHOUT selecting it — a real click/keyboard-select is still required to commit the choice.
...
- Build item 4: a "preview" state distinct from "selected" state for every carousel instance (departures, arrivals, calendar — the `strip_id`-per-instance discipline from 27-07 must be preserved so no two carousels share preview state), likely via `IntersectionObserver` watching chip centering within the scroll container. Respect the CSP constraints already on record (no inline script, `img-src 'self' data:` — no blob: URLs).
- Fix item 5: replace the hamburger glyph with a gear icon on `#site-nav-toggle` only; keep the `aria-label` and panel contents unchanged since the label is already accurate.

**Carried discipline from Phase 27:** every new check must assert relationships, not just endpoints (item 2's readout fix is proven by the caption's actual displayed text matching computed HH:MM + duration after an interaction, not by the existence of a format function; item 2's handle fix is proven by SAMPLING the handle's resolved position throughout a held press, not only before and after; item 4's fix is proven by the preview `<img src>` actually matching whichever chip is geometrically centered during a scroll, not by a scroll listener being attached). Mutation-test every new check, with the failure message quoted. `git checkout-index -f --` for any mutation revert, never `git checkout --`. `EXPECTED_CHECK_COUNT` re-derived by running. **Item 3's resilience work must not overclaim**: it is proven as "a failure is always visible and recoverable," never as "the Safari save bug is fixed" — that claim would be false, since the root cause remains unconfirmed.

**Addendum, 2026-09-16 — item 3 reversed after real-device evidence changed the picture.** With 28-01 through 28-04 shipped, the developer got hold of a Mac and captured the real Safari Network tab for a settings save: `POST /settings` → **204**, every field present (`tracked_runway: 02-20`, `quiet_hours_start: 23:00`, etc.), and confirmed after reload that the value genuinely persisted. **The auto-save mechanism was never broken — it worked correctly the whole time; the only real defect was that nothing ever visibly confirmed it**, exactly the class of problem CFG-74's resilience work (scroll-independent status, failure-only retry) was built to paper over without knowing the cause. Having seen this, the developer rejected the auto-save model itself rather than accepting the resilience fix: *"Mais ce n'était pas le comportement d'enregistrement qu'on a choisi. Je veux garder la pop up qui apparait et qui propose d'enregistrer."* Asked to clarify against Phase 27's own removal of exactly this bar (with a direct quote-back to the developer's own original complaint, since the ask sounded like a contradiction), the developer confirmed unambiguously: they want the **pre-Phase-27 dirty save bar** restored — a bar naming which section(s) changed, with real, clickable **Enregistrer**/**Annuler** buttons, using the old native-form-POST model (never the fetch-based auto-save) — and that this must be the **only** save affordance on the settings pages, nowhere else. Confirmed via two follow-up questions: (1) Calendar Connect/Replace and Airlines' own "Enregistrer le nom" button are explicitly OUT of scope, since they're different actions on different data with their own dedicated forms, not settings edits; (2) the bar restoration takes the confirmed shape with real buttons, not a button-less "prominent status only" alternative.

**CFG-63 is retired in place** (the requirement it recorded — no button, silent auto-save — really was built and shipped; it is the product decision that reversed, not the evidence). **CFG-64 keeps its tick, amended only in framing** — its substance (the native submit emitted unconditionally, provable off disk with scripts blocked) survives entirely intact and becomes the mechanism the restored bar's own Save control uses. **CFG-74 is superseded before ever being built** — 28-06 and 28-07 (the plans that would have implemented it) never executed; their placeholder scope is replaced below. **CFG-77 and CFG-78** carry the restoration.
...
**Plans:** 8 plans, 8 waves (strictly sequential — `config_page.py` and `test_config_page.py` are touched by nearly every plan, so there is no parallelism to take). The pre-planning `/gsd-sketch` round is CLOSED: two throwaway HTML variants on the real 18-theme registry and the developer's last flight, decided on the visible result — Variant A (Accordion) won, developer-approved 2026-09-22 (`.planning/sketches/006-aspect-tile-accordion-vs-segments/`), and `30-UI-SPEC.md` is the design contract derived from it.

Plans:
**Wave 1**

- [ ] 30-01-PLAN.md — Playwright + Chromium installed; the genuine same-instrument CFG-86 "before" reading at 390px and 360px against the unmodified tree (wave 0)
- [ ] 30-02-PLAN.md — the palette's new renderers: the CSS-drawn swatch (5 of 18 banded, 13 solid, derived from the live registry), the palette chip and grid, the row summary — additive, tested, called by nothing yet (wave 1)

**Wave 2** *(blocked on Wave 1 completion)*

- [ ] 30-03-PLAN.md — retire the pins before the mechanisms they pin: 3 config-page checks, 5 browser checks, the shared strip-selector block and the no-JS registry's pagers row, with a machine-guarded re-pin ledger for every property owed to a later plan (wave 2)

**Wave 3** *(blocked on Wave 2 completion)*

- [ ] 30-04-PLAN.md — the cut: `_aspect_card_html()` replaces the four-row radiogroup and three carousels, 16 constants and 4 builders retired, the French catalogue updated, the CFG-86 instrument repointed in the same commit as the heading rename, `theme-preview.js` re-scoped to `.aspect-card` (wave 3)

**Wave 4** *(blocked on Wave 3 completion)*

- [ ] 30-05-PLAN.md — re-pin the accordion: 8+ replacement checks asserting relationships and deriving every count from the registry; zero ledger rows owed by 30-05 (wave 4)

**Wave 5** *(blocked on Wave 4 completion)*

- [ ] 30-06-PLAN.md — the calendar absorbed into its own row in the sketch's flatter shape; `calendar_group()` and 3 constants retired; one render gate with the registry invariant documented at the code; `ASPECT_CAPTION_EXEMPTIONS` down to two (wave 5)

**Wave 6** *(blocked on Wave 5 completion)*

- [ ] 30-07-PLAN.md — CSS: 15+ new rules on existing tokens only, the audited deletions with three-category pre-delete greps in `30-CSS-AUDIT.md`, and the three-phase-old accent-reservation-list gap closed (wave 6)

**Wave 7** *(blocked on Wave 6 completion)*

- [ ] 30-08-PLAN.md — the preview follows hover and focus (new code, no analog); scripts-blocked accordion operability and save-to-disk at 360px in both languages; every touch target measured in both themes; CFG-86's after reading, delta and verdict (wave 7)
