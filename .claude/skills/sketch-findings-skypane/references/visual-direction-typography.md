...
- **Battery trend section:** still current — moved out of the stat-tile dashboard grid into its own full-width section, with the same card contract (now hairline-at-rest, not shadow-at-rest — see above) and a serif `<h2>` heading. The heading carries no glyph (quick task 260902-j8w) — see the "Icons on stat tiles" bullet above for the round trip.
