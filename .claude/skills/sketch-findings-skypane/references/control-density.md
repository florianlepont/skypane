...
/* Selected-card treatment, fallback layer: border (unchanged) + background
 * wash (06.6.4.1.1-06 addition). Byte-for-byte unchanged since — this is
 * now the no-:has() FALLBACK, not the live-state mechanism (260904-bbi). */
...
@supports selector(:has(*)) {
  ...
  .theme-chip--selected:not(:has(input:checked)) {
    border: 2px dashed color-mix(in srgb, var(--color-text) 70%, transparent);
  }
