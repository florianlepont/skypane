 ...
 * CONFIG_FP_MIN_REFRESH_SPACING_S between refreshes, never starts a blit
 * while one is running, and WAITS out the spacing (up to
 * CONFIG_FP_MAX_GUARD_WAIT_S) rather than discarding an image already in
 * hand — see panel_guard.h. ESP_ERR_INVALID_STATE: a blit is already
 * running. ESP_ERR_TIMEOUT: the spacing outlasts the awake budget; the
 * caller must NOT record the hash, so the next wake redraws. */
