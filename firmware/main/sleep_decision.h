 ...
 * This module does not re-validate server_sleep_s's upper bound — that
 * is validate.c's fp_sleep_s_parse job, run before this is ever called. */
#pragma once
#include <stdbool.h>
#include <stdint.h>
...
/* Decides the sleep plan for one wake. FAILED, or server_sleep_s == 0
 * under any outcome: a zero-second timer wake would hot-loop the radio,
 * so this counts as a failure too — exponential backoff
 * (fp_backoff_seconds(backoff_n)), counter incremented (saturating at
 * UINT8_MAX). OK with a non-zero server_sleep_s: sleep exactly that
 * long, reset the counter to 0. DEFERRED with a non-zero
 * server_sleep_s: sleep for server_sleep_s, unless panel_wait_s is
 * shorter and non-zero, in which case sleep for panel_wait_s + 5 (never
 * longer than server_sleep_s) — this resets the counter too, since a
 * deferred draw is a healthy wake, not a failure. */
