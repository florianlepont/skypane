...
- `main/battery_math.h` / `main/battery_math.c` — SkyPane-original, not
  ...
  Introduced in plan `05-03` (Phase 5, DEVICE-04).
...
`main/battery.c`'s `battery mv=<pack> pin_mv=<pin>` line, emitted with log
tag `fp_batt`, is a **diagnostic addition outside this five-line contract**
(Phase 5, DEVICE-04, plan `05-03`). `hardware/logtools.py`'s `check-battery`
reads captured stub/production-server stdout, not device console output, so
this line's tokens are deliberately distinct from `X-Battery-Mv` to avoid
any risk of a device-console capture being confused for a server-log
capture.
