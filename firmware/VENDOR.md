...
- `main/fault_screen.c` / `main/fault_screen.h` — SkyPane-original, not
  vendored; upstream has no on-device fallback rendering at all (every
  panel update upstream ships is server-rendered). Pure C11, no ESP-IDF
  dependency: renders the NO CONNECTION hold screen (DEVICE-06) entirely
  in firmware — an on-device integer Floyd-Steinberg dither of the same
  field DISPLAY OFF/QUIET HOURS/BATTERY EMPTY use server-side, stamped
  with a committed ink mask — plus the allow-listed step/backoff-counter/
  already-shown gate (`fp_fault_screen_should_draw()`) that decides
  whether to draw at all. Wired into `main/app_main.c`'s
  `fail_and_sleep()`. Introduced in quick task `260924-u7n`
  (`.planning/seeds/on-device-fault-icon.md`).
- `main/fault_screen_mask.h` — SkyPane-original, not vendored; generated
  (never hand-edited — `server/test_fault_screen_mask.py` proves it
  matches its generator byte-for-byte) 1-bpp ink mask of
  `server/plane/render.py`'s `_build_no_connection_canvas(flat=True)`
  composition, produced by `tools/gen_fault_screen.py`. Introduced in
  quick task `260924-u7n`.
- `tools/gen_fault_screen.py` — SkyPane-original, not vendored; upstream
  has nothing to generate, since it never renders on-device. Renders
  `server/plane/render.py`'s NO CONNECTION composition flat, extracts and
  packs the ink mask into `main/fault_screen_mask.h`, and implements a
  Python port of `fault_screen.c`'s exact integer Floyd-Steinberg dither
  spec (one spec, two implementations) to produce a firmware-equivalent
  side-by-side preview PNG. Introduced in quick task `260924-u7n`.
...
| `fp_diag` | `fault screen skipped: no PSRAM` | `heap_caps_malloc()` for the fault-screen framebuffer returned `NULL` — the fault screen is skipped for this wake and sleep proceeds normally. Introduced in quick task `260924-u7n`. |
