...
| H-18 | FW-15 | Prod: the server log's `telemetry: X-Fw-Version=` value equals H-01's `Firmware version:` line. Dev: the LAN stub's `telemetry:` lines show `X-Fw-Version = <git describe>-dev`. | Prod: 9 × `X-Fw-Version=e8d293c` in the VPS journal. Dev: 55 × `X-Fw-Version=e8d293c-dev` in `H-08-lan-dev-server.log`. | see `H-04-vps-after-server.log` (prod) / `H-08-lan-dev-server.log` (dev) | PASS |
